...
carries its provenance, its normativity and a stable id — such that an on-prem system can merge it with
...
- **`REG-N01` and `REG-N02` have never had their source files.** They are the rows the whole 800-171
  ...
  that SEP-authored JSON exists, **the corpus has no 800-171 control text at all**, and the QA gates that
